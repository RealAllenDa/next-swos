<template>
  <q-toolbar
    class="q-pl-xl q-pr-xl row q-mb-lg q-mt-lg q-pb-sm"
  ...
  >
    ...
    <q-separator spaced="lg" vertical></q-separator>
    ...
    <q-btn class="q-mr-lg" color="primary" icon="speed" outline>
      <q-tooltip
        :offset="[0, 20]"
        anchor="top middle"
        class="text-bold"
      ...
      >
        <template v-slot:label>
          <q-tooltip
            ...
            transition-duration="0"
          >
            {{ precipitationStore.selectedDuration.replace('h', '') }}-hr.
            precipitation
          </q-tooltip>
        </template>
        <q-list>
          ...
          >
            <q-item-section>
              <q-icon
                v-if="
                  precipitationStore.rainMeasurementsDisplayOption === 'text'
                "
                class="q-mr-md"
                name="check"
                size="24px"
...
export default defineComponent({
  ...
  setup() {
    ...
    watch(isInPlayback, () => {
      if (!sliderInitialized.value) return;
