...
export const usePrecipitationStore = defineStore('precipitation', {
  ...
  getters: {
    ...
    thumbnailQualityResolution: (state) => {
      return (
        state.resolutionOptions.find((option) => option.thumbnail)?.value ??
        state.resolutionOptions[0]?.value ??
  ...
  actions: {
    ...
    setOptions(
      ...
      torrential: boolean | undefined,
      gpv: boolean | undefined,
      station: boolean | undefined
    ) {
    ...
    initDataList(mutationType: string) {
      ...
      if (this.mapOptions[this.mapId] === undefined) {
        ...
        if (mutationType === 'duration') {
          ...
          sdk.showNotification(
            'negative',
            `Exhaustive handling of mutationTypes: ${mutationType}`
          );
          return;
        }
      }
      if (this.mapOptions[this.mapId] === undefined) {
        sdk.showNotification(
          'negative',
          `No map configuration exists for ${this.mapId}`
        );
        return;
      }
      ...
      if (data === undefined || data === null) {
        sdk.showNotification(
          'negative',
          `Shouldn\'t happen: data is null of dataId ${dataId}`
        );
        return;
      }
    ...
    initRainMeasurements() {
      this.rainMeasurementsAvailable =
        this.mapOptions[this.mapId].rain_measurements_avail;
    ...
    setCurrentData() {
      ...
      if (data === undefined) {
        sdk.showNotification(
          'negative',
          "Shouldn't happen! currentList[currentTime] is null"
        );
        return;
      }
